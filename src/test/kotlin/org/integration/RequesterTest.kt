...
class RequesterTest {
    ...
    @Test
    fun `can successfully make a request to the Wikipedia API`() = runTest(timeout = timeout) {
        val httpResponse = BaseRequester.doWikipediaRequest("") {}
        assertEquals(200, httpResponse.status.value)
    }

    @Test
    fun `can successfully make a request to the Wikidata API`() = runTest(timeout = timeout) {
        val httpResponse = BaseRequester.doWikidataRequest("") {}
        assertEquals(200, httpResponse.status.value)
    }

    @Test
    fun `can successfully fetch a QID from Wikidata given a name`() = runTest(timeout = timeout) {
    ...
    @Test
    fun `can successfully fetch a label and claims from Wikidata given a QID`() = runTest(timeout = timeout) {
