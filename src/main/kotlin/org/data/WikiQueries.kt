...
suspend fun getFamilyInfo(wikidataId: String): Map<String, List<String>> {
    val response = doWikidataRequest("wbgetclaims") {
    ...
    claims?.forEach { (prop, claimDetails) ->
        if (prop in familyProps.keys) {
            val familyMembers = claimDetails.jsonArray.mapNotNull { claim ->
                claim.jsonObject["mainsnak"]?.jsonObject
    ...
    familyProps.values.forEach { relation ->
        familyInfo.putIfAbsent(relation, mutableListOf())
    }
...
suspend fun convertWikidataIdsToNames(familyInfo: Map<String, List<String>>): Map<String, List<String>> {
    ...
    val response = doWikidataRequest("wbgetentities") {
    ...
    entities?.forEach { (id, details) ->
        val label = details.jsonObject["labels"]?.jsonObject
            ?.get("en")?.jsonObject
    ...
    return familyInfo.mapValues { (_, ids) ->
        ids.map { id -> idToNameMap[id] ?: "Unknown" }
    }
}
...
suspend fun main() {
    ...
    familyMembers.forEach { (relation, names) ->
        println("$relation -> $names")
    }
}